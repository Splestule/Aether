...
export function setupRoutes(app: Express, services: Services) {
  ...
  app.post('/api/opensky/reconnect', async (req, res) => {
    const sessionToken = getSessionToken(req);
    const authService =
      isBYOKEnabled && byokSessionService && sessionToken
    ...
    try {
      const header =
        isBYOKEnabled && sessionToken
          ? await (authService as BYOKAuthService).getAuthorizationHeader(sessionToken, {
              forceRefresh: true,
            })
          : await (authService as OpenSkyAuthService).getAuthorizationHeader({
              forceRefresh: true,
            });
