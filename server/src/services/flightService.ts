...
export class FlightService {
  ...
  async getFlightsInArea(
    ...
    logger.action(
      ...
      'Attempting to fetch real flight data from OpenSky API...'
    );
    ...
    try {
      ...
      const rawFlights = await this.fetchFromOpenSky(bbox, sessionToken);
      ...
      logger.action(
        'Flights demo fallback',
        `Using ${demoFlights.length} demo flights as fallback`
      );
      return demoFlights;
    }
  }
   ...
   */
  async getFlightByIcao(icao: string, sessionToken?: string): Promise<ProcessedFlight | null> {
   ...
   */
  private async fetchFromOpenSky(
    bbox: {
      minLat: number;
      maxLat: number;
      minLon: number;
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
          ...
          try {
            ...
            const authHeader =
              sessionToken && this.authService instanceof BYOKAuthService
                ? await this.authService.getAuthorizationHeader(sessionToken, {
                    forceRefresh: authAttempt > 0,
                  })
                : await (this.authService as OpenSkyAuthService)?.getAuthorizationHeader({
                    forceRefresh: authAttempt > 0,
                  });
            if (authHeader) {
            ...
            const flights: FlightData[] = response.data.states.map(
              (state): FlightData => ({
        ...
        if (attempt === this.MAX_RETRIES) {
          throw new Error(
            `Failed to fetch flight data after ${this.MAX_RETRIES} attempts: ${lastError.message}`
          );
        }
      ...
      for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
        try {
          ...
          while (authAttempt < this.AUTH_RETRY_LIMIT) {
            try {
              ...
              if (!response.data || !Array.isArray(response.data.path)) {
                logger.action(
                  'OpenSky trajectory empty',
                  'No trajectory data received from OpenSky API'
                );
                return [];
              }
          ...
          if (axios.isAxiosError(error)) {
            logger.error(
              'E-FLT-004',
              `Trajectory attempt ${attempt} failed. Status: ${error.response?.status}`,
              {
          ...
          if (attempt === this.MAX_RETRIES) {
            throw new Error(
              `Failed to fetch flight trajectory after ${this.MAX_RETRIES} attempts: ${lastError.message}`
            );
          }
  ...
  getStats(sessionToken?: string): {
    ...
    openskyAuthDetails?:
      | ReturnType<OpenSkyAuthService['getStatus']>
      | ReturnType<BYOKAuthService['getStatus']>;
  } {
    ...
    return {
      ...
      openskyAuthentication: hasCreds ? 'authenticated' : 'anonymous',
      openskyAuthDetails: authDetails,
    };
  }
}
