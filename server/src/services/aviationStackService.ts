...
export class AviationStackService {
  ...
  constructor(private readonly cacheService: CacheService, options?: AviationStackServiceOptions) {
    this.apiKey = process.env.AVIATIONSTACK_API_KEY
    this.cacheTtl = options?.ttlSeconds ?? 300
    this.baseUrl = options?.baseUrl ?? process.env.AVIATIONSTACK_API_URL ?? 'http://api.aviationstack.com/v1'
  }
  ...
  private parseResponse(response: AviationStackResponse, callsign: string): FlightRouteInfo | null {
    ...
    const flight = response.data?.find(item => {
