...
import { gpsToVRCoordinates, calculateDistance, calculateBearing, calculateElevation } from '@vr-flight-tracker/shared'
...
export class DemoService {
  ...
  private generateDemoFlights() {
    const airlines = [
      ...
      'Austrian Airlines', 'SAS', 'Finnair', 'Iberia', 'TAP Air Portugal',
      'Brussels Airlines', 'Eurowings', 'Ryanair', 'easyJet', 'Wizz Air'
    ]

    const callsigns = [
      'LH123', 'BA456', 'AF789', 'KL012', 'LX345',
      'OS678', 'SK901', 'AY234', 'IB567', 'TP890',
      'SN123', 'EW456', 'FR789', 'U2012', 'W6345'
    ]
  ...
  updateFlights() {
    this.demoFlights.forEach(flight => {
      ...
      const newLon = flight.gps.longitude + (speed * Math.cos(headingRad)) / (111 * Math.cos(flight.gps.latitude * Math.PI / 180))
